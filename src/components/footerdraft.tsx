...
export default function FooterDraft({ time, once, appstore, playstore }) {
  ...
  return (
    <Div>
      <motion.div initial="hidden" whileInView="show" viewport={{ once: once, amount: 0.5 }} onViewportLeave={() => sethref(false)}>
        <motion.ul variants={container}>
          <Div flex flexRow>
                ...
                <Div relative>
                  <Div absolute _translateY1over2 _translateX1over2>
                    <motion.li variants={bgAnimation}>
                      <Div imgTag src={IMAGES.bwLogo.bg} />
                    </motion.li>
                  </Div>
                  <Div absolute _translateY1over2 _translateX1over2>
                    <motion.li variants={contentAnimation}>
                      <Div imgTag src={IMAGES.bwLogo.content} />
                    </motion.li>
                  </Div>
                </Div>
              </Div>
            ...
            <Div flexCol selfCenter itemsCenter>
              <motion.li variants={text1Animation}>
                <Div imgTag h70 src={IMAGES.logoword.firstBlack} />
              </motion.li>
              <motion.li variants={text2Animation}>
                <Div mt30 imgTag h={22.5} src={IMAGES.logoword.secondGradientNew} />
              </motion.li>
            </Div>
          </Div>
          <motion.li variants={linkAnimation}>
            <Div flex justifyCenter mt50 gapX={20}>
              {canhref ? (
                <Div imgTag h50 src={IMAGES.downloadOnAppStore} cursorPointer onClick={appstore} />
              ) : (
                <Div imgTag h50 src={IMAGES.downloadOnAppStore} />
              )}
              {canhref ? (
                <Div imgTag h50 src={IMAGES.downloadOnGooglePlay} cursorPointer onClick={playstore} />
              ) : (
                <Div imgTag h50 src={IMAGES.downloadOnGooglePlay} />
              )}
            </Div>
          </motion.li>
        </motion.ul>
