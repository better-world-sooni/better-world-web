...
function UserList() {
  ...
  const {
    ...
    data: user_list,
    refetch,
  } = getUserListQuery(page_size, offset, search_key, () => setLoadingButton(true));
  ...
  return (
    <Div flex flexCol>
      ...
      {user_list?.success && (
        <Div selfCenter>
          <Pagination
            count={Math.ceil(user_list?.list?.total_length / page_size)}
            page={offset + 1}
            showFirstButton
            showLastButton
          ...
          <Disclosure.Button className="w-full text-gray-400 hover:bg-gray-100 hover:text-gray-500">
            <Div py12 px16 wFull flex flexRow cursorPointer clx={`${HandleOpen(open) ? "bg-gray-100 text-gray-500" : ""}`}>
              <Div justifyItemsStart wFull flex flexRow>
                <Div fontSize20 textBlack minW={150} maxW={150} mr10 textLeft fontBold overflowEllipsis overflowHidden whitespaceNowrap>
                  {user.user_info.user_name ? user.user_info.user_name : user.user_info.user_nft_name}
                  <br></br>
                  <Div fontSemibold fontSize13 overflowEllipsis overflowHidden whitespaceNowrap>
                    {user.user_info.user_name ? user.user_info.user_nft_name : <EmptyBlock h={20} />}
                  </Div>
                </Div>
                <Div flex flexRow flexWrap>
                  <DataEntry
                    name={"PFP 개수"}
                    w={55}
                    label={<CubeIcon height={20} width={20} className="max-h-20 max-w-20 mr-10" />}
                    data={user.user_info.pfp_count}
                  />
                  <DataEntry
                    name={"게시물 합"}
                    w={55}
                    label={<PencilAltIcon height={20} width={20} className="max-h-20 max-w-20 mr-10" />}
                    data={user.user_info.posts}
                  />
                  <DataEntry
                    name={"제안 합"}
                    w={55}
                    label={<LightBulbIcon height={20} width={20} className="max-h-20 max-w-20 mr-10" />}
                    data={user.user_info.proposals}
                  />
                  <DataEntry
                    name={"기여도 합"}
                    w={65}
                    label={<HeartIcon height={20} width={20} className="max-h-20 max-w-20 mr-10" />}
                    data={user.user_info.contribution}
                  />
                </Div>
              </Div>
          ...
          <DefaultTransition
            ...
            content={
              <Disclosure.Panel static className="bg-white bg-gray-100 border-b-2">
                <UserAddressPanel user_address={user.user_address} superPrivilege={user?.super_privilege} />
        ...
        <>
          <Disclosure.Button className="w-full bg-gray-100 text-gray-400 hover:bg-gray-200 hover:text-gray-500">
            <Div py12 px16 wFull flex flexRow cursorPointer clx={`${open ? "bg-gray-200 text-gray-500" : ""}`}>
              ...
              <Div justifyItemsStart wFull flex flexRow>
                <Div fontSize20 selfCenter textBlack minW={150} maxW={150} mr25 textLeft fontBold>
                  <Div flex flexRow>
                  ...
                  <Div fontSemibold fontSize13>
                    {nft.name ? nft.nft_name : <EmptyBlock h={20} />}
                  </Div>
                </Div>
                <Div flex flexRow flexWrap>
      ...
      <Div wFull flex flexRow>
        <Div justifyItemsStart wFull flex flexRow flexWrap>
          ...
          <DataEntry
            ...
            label={
              <Div flex flexRow>
                <CogIcon height={20} width={20} className="max-h-20 max-w-20 mr-10 mb-20" />
                Admin
              </Div>
            }
            data={
          ...
          <Div selfCenter mb10 justifyItemsStart flex flexRow ml25 mr25>
            <Div>
              <Div px10 mt10 cursorPointer py5 bgGray400 rounded10 clx="hover:bg-gray-600 hover:text-white" onClick={handleGetPosts}>
                {" "}
                게시물 관리
              </Div>
              <EmptyBlock h={20} />
...
function UserAddressPanel({ user_address, superPrivilege }) {
  ...
  const queryClient = useQueryClient();
  const { Modal, openModal, isLoading, mutate } = MakeSuperPrivilegeModal(user_address, !superPrivilege, queryClient);
    ...
    <>
      ...
      <Div py5 fontSize13 bgGray100 px20 flex flexRow>
        ...
        <Tooltip title="클릭 시 복사" arrow>
          <Div py5 h30 rounded10 px10 cursorPointer bgGray300 onClick={openCopied}>
            {user_address}
          </Div>
        </Tooltip>
...
export function SwitchToggle({ checked, onChange, lock = false }) {
  return (
    <Switch
      ...
      className={`${
        checked ? (lock ? "cursor-default bg-gray-600" : "cursor-pointer bg-bw") : lock ? "cursor-default bg-gray-400" : "cursor-pointer bg-secondary"
      }
