	...
	return (
        ...
        <Div absolute minW={200} maxW={300} rounded px20 bgWhite bgOpacity90 borderR1>
            ...
            {frame.map((frame, index) => (
                <Div key={index} textBlack bgWhite fontSize20 px5 rounded10 py5 justifyCenter cursorPointer clx={"hover:bg-gray-100"}onClick={() => {defaultpatch();setaadminComponent(frame)}}>
                    {frame.props.name}
                </Div>
            ))}
        </Div>
