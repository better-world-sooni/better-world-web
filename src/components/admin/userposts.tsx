...
export default function UserPostModal({ name, contract_address, token_id }) {
  ...
  return (
    <Modal
      open={UserPostEnabled && contract_address_reducer == contract_address && token_id_reducer == token_id}
      onClose={closeModal}
      bdClx={"bg-black/50"}
      clx={"bg-white w-full"}
    >
...
function ModalEntry({ name, contract_address, token_id, closeModal }) {
  ...
  const {
    ...
    data: post_list,
    refetch,
  } = getUserPostListQuery(contract_address, token_id, page_size, offset, search_key, () => {
      ...
      <Div wFull mb10 flex flexCol px30 py30>
        <Div wFull flex flexRow py10 borderB1>
          <Div wFull selfCenter fontBold fontSize30 selfStart textBW>
            {name}
            <Div spanTag fontSemibold fontSize25 textBlack>
              님의 게시글
            </Div>
          </Div>
          <Div selfCenter>
            ...
            {post_list?.success && (
              <Div selfCenter wFull flex flexCol>
                <Div wFull selfCenter>
                  <MakePostList post_list={post_list} />
                </Div>
                <Div selfCenter>
                  <Pagination
                    count={Math.ceil(post_list?.list?.total_length / page_size)}
                    page={offset + 1}
                    showFirstButton
                    showLastButton
