...
import { RefreshIcon, ChevronUpIcon, HeartIcon, AnnotationIcon, ThumbUpIcon, ThumbDownIcon, TrashIcon, LightBulbIcon, PencilAltIcon, XIcon, CheckIcon } from "@heroicons/react/outline";
...
export default function PostList({post_list, showEntry=false, maxCommentLength=2, DeletePostAction=null, DeleteCommentAction=null}) {
	...
	if (list.length==0) return (<Div wFull textCenter textGray400>작성한 글이 없습니다.</Div>);
	return (
		<Div wFull border1>
			{list.map((post, _) => (
          ...
          {({ open }) => (
			...
			<DefaultTransition show={open} content={
              <Disclosure.Panel className="bg-white bg-gray-100 border-b-2">
				<Div bgGray100 textGray700><PostDetails post={post} showEntry={showEntry} maxCommentLength={maxCommentLength} postModal={postModal} CommentModal={CommentModal}/></Div>
              </Disclosure.Panel>
			}/>
            </>
          )}
	...
	return (
		...
		<Div flex flexCol px10 py10>
			<Div px10 py10 selfStart flex flexRow wFull>
				...
				{showEntry && <Div selfCenter flex flexCol>
				<Div selfEnd mr20 textRight fontSize12>{getDate(post.updated_at)}</Div>
				...
				{!showEntry && 
				<Div selfCenter mr25 textRight minW={150}>{getDate(post.updated_at)}</Div>}
...
function Comment({comment, maxLength, CommentModal}) {
	...
	return (
		...
		<Div wFull flex flexRow>
				...
				<Div selfCenter flex flexCol>
				<Div selfEnd mr20 textRight fontSize12><Date updated_at={comment.updated_at}/></Div>
...
function ForVotesDataEntry({post, w, labelsize}) {
	return (
		post.type=='Proposal' && <DataEntry name={"찬성 수"} w={w} label={post.for_votes_count>0 ? <ThumbUpIcon height={labelsize} width={labelsize} className="max-h-20 max-w-20 mr-10 text-success" />:<ThumbUpIcon height={labelsize} width={labelsize} className="max-h-20 max-w-20 mr-10" /> } data={post.for_votes_count}/>
	)
}

function AgainstVotesDataEntry({post, w, labelsize}) {
	return (
		post.type=='Proposal' && <DataEntry name={"반대 수"} w={w} label={post.against_votes_count>0? <ThumbDownIcon height={labelsize} width={labelsize} className="max-h-20 max-w-20 mr-10 text-warning" />:<ThumbDownIcon height={labelsize} width={labelsize} className="max-h-20 max-w-20 mr-10" /> } data={post.against_votes_count}/>
	)
}

function LikeDataEntry({post, w, labelsize}) {
	return (
		<DataEntry name={"좋아요 수"} w={w} label={post.likes_count>0 ? <HeartIcon height={labelsize} width={labelsize} className="max-h-20 max-w-20 mr-10 text-danger" /> : <HeartIcon height={labelsize} width={labelsize} className="max-h-20 max-w-20 mr-10" />} data={post.likes_count}/>
	)
}

function CommentsDataEntry({post, w, labelsize}) {
	return (
		<DataEntry name={"댓글 수"} w={w} label={post.comments_count>0 ? <AnnotationIcon height={labelsize} width={labelsize} className="max-h-20 max-w-20 mr-10 text-gray-400" /> : <AnnotationIcon height={labelsize} width={labelsize} className="max-h-20 max-w-20 mr-10" />} data={post.comments_count}/>
	)
}
...
function DeleteButton({Loading, openModal}) {
    return (
        Loading ? <Div selfCenter px10 py5 bgDanger bgOpacity50 rounded10 textWhite> <Oval height="14" width="14" color="red" secondaryColor="#FFFFFF" strokeWidth="5" /></Div> :
        <Div selfCenter px10 py5 bgDanger bgOpacity50 rounded10 textWhite cursorPointer clx="hover:bg-danger" onClick={openModal}> <TrashIcon height={20} width={20} className="max-h-20 max-w-20" /></Div>
    )
}
